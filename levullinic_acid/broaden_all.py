...
folder_names = ['levullinic_acid_%d'%ii for ii in range(1) ]
...
for ii,folder_name in enumerate(folder_names):
    ...
    np.savetxt('levullinic_acid_broad_%02d.dat'%(ii+1), data[folder_name])

master_x = data[folder_name][:,0]
...
np.savetxt('levullinic_acid_broad_av.dat', data['mean'])
